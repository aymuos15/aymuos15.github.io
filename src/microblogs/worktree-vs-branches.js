...
function buildWorktreeDiagram() {
  ...
  const branchLayout = {
    ...
    labels: [
      { dim: true, text: "main", x: 62, y: 130 },
      { dim: false, text: "feat-A", x: W / 2, y: 190 },
      { dim: true, text: "fix-B", x: 378, y: 130 },
    ],
  };
  ...
  function buildSvg(layout) {
    ...
    for (const [x1, y1, x2, y2] of layout.connectors) {
      s += `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}"
                stroke="var(--text-secondary)" stroke-width="0.9" stroke-dasharray="4 3" opacity="0.45"/>`;
    }

    // Boxes
    const single = layout.boxes.length === 1;
    for (const [i, b] of layout.boxes.entries()) {
      const col = !single && i === 0 ? "var(--link)" : "var(--text)";
      s += `<rect x="${b.x - bw / 2}" y="${b.y - bh / 2}" width="${bw}" height="${bh}"
                rx="4" fill="none" stroke="${col}" stroke-width="1.2"/>`;
      let inner = "working dir";
      if (!single) {
        inner = i === 0 ? "feat-A/" : "fix-B/";
      }
  ...
  caption.textContent =
    "Click to compare single-directory branching with parallel worktrees.";
