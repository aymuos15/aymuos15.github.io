...
(function navTilesModule() {
  ...
  const PASTELS = window.pastelColors?.length
  ...
  function hash(x, y) {
    const r = Math.sin(x * 127.1 + y * 311.7) * 43_758.5453;
  ...
  class NavTileField {
    ...
    build() {
      ...
      s.fillStyle = "#000";
      for (const a of this.container.querySelectorAll("a")) {
      ...
      const { data } = s.getImageData(0, 0, ow, oh);
    ...
    frame(t) {
      ...
      const { ctx, viewW, viewH, cell, n, px, py, spark, lit, seed } = this;
      ...
      const { lightSpeed } = this;
      const moving = this.hasPointer;

      const reach = clamp(
        viewH * 0.22 + lightSpeed * 0.9,
        viewH * 0.18,
        viewH * 0.42
      );
      ...
      for (let idx = 0; idx < NP; idx += 1) {
        ...
        for (let as = 0; as < ALPHA_STEPS; as += 1) {
      ...
      for (const i of litList) {
        ...
        const gh = gsz / 2;
        const [rr, gg, bb] =
          PASTEL_RGB[Math.min(NP - 1, Math.floor(this.hueSeed[i] * NP))];
        if (spark[i]) {
          const ph = seed[i];
          const tt = 0.000_25 * t;
  ...
  (function initNavTiles() {
    ...
    let field = null;
    const shouldRun = () =>
      about.classList.contains("active") &&
      container.classList.contains("open");

    const update = () => {
      if (shouldRun()) {
        if (!field) {
          field = new NavTileField(container);
        }
        field.activate();
      } else {
        field?.deactivate();
    ...
    obs.observe(about, { attributeFilter: ["class"], attributes: true });
    obs.observe(container, { attributeFilter: ["class"], attributes: true });
