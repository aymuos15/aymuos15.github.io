...
class Typer {
  ...
  build() {
    ...
    this.charNodes = [];
    const parts = this.source.split(WHITESPACE_RUN);
    let i = 0;
    for (const part of parts) {
      ...
      for (const ch of part.split("")) {
        ...
        span.style.setProperty(
          "--char-color",
          window.pastelColors[
            Math.floor(Math.random() * window.pastelColors.length)
          ]
        );
        this.charNodes.push({ cp, currentClass: "char charInit", el: span });
        i += 1;
  ...
  applyFrame() {
    ...
    for (const node of this.charNodes) {
      ...
      let cls = variation ? `char ${variation}` : "char";
      if (p <= 0) {
        cls = "char charInit";
      } else if (p >= 1) {
        cls = "char";
