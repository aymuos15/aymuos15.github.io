...
function _shade(hex, f) {
  // hex is "#rrggbb"; each channel is read straight off the string.
  const channel = (at) =>
    Math.min(255, Math.trunc(Number.parseInt(hex.slice(at, at + 2), 16) * f));
  ...
  function drawFrame(progress) {
    ...
    for (let row = 0; row < nR; row += 1) {
      for (let col = 0; col < nC; col += 1) {
        ...
        ctx.lineTo(sx + CW, by);
        ctx.closePath();
        ctx.fillStyle = _shade(c, 0.65);
        ctx.fill();

        hits.push({ day, h, sx, ty });
      }
    }
...
contribCanvas.addEventListener("mousemove", (e) => {
  ...
  const sc =
    (Number.parseFloat(contribCanvas.style.width) || rect.width) / rect.width;
  ...
  for (let i = hits.length - 1; i >= 0; i -= 1) {
    ...
    if (mx >= sx && mx < sx + CW && my >= ty && my < ty + h) {
      hit = hits[i];
      break;
    }
  }
...
contribCanvas.addEventListener("mouseleave", () => {
  contribTip.style.display = "none";
});

async function _loadContribs() {
  const res = await window.fetch(
    "https://github-contributions-api.jogruber.de/v4/aymuos15?y=last"
  );
