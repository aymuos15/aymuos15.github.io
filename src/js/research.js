...
const researchThemes = [
  {
    ...
    interactive: "niivue",
    intro:
      "The core of my PhD is segmenting neuro-oncology images at every granularity. Drag, scroll and switch modality on a real BraTS multi-metastasis case below.",
    orb3d: "head",
    papers: [
      {
        title: "SegPatch",
  ...
  },
  {
    ...
    intro:
      "Overlap metrics favour big structures, so tiny lesions barely dent the score. I build instance-aware losses that weight every lesion equally. Toggle to see the effect.",
    orb3d: "sphere",
    papers: [
      {
        title: "Cluster Dice",
        url: "https://github.com/aymuos15/Cluster-Dice",
        venue: "SPIE Medical Imaging 2025",
      },
      {
        title:
          "Instance Awareness of Multi-class Semantic Segmentation Loss Functions",
        url: "https://github.com/BrainImageAnalysis/instance-loss",
        venue: "CVPR CV4Clinical Workshop 2026",
      },
    ],
    tint: "#8b9bd4",
...
(() => {
  ...
  const center = (N - 1) / 2;
  for (const [i, theme] of researchThemes.entries()) {
    ...
    if (theme.orb3d === "head") {
      orbHtml =
        '<span class="research-card-orb research-card-orb--scan"><canvas class="research-card-orb-scan" width="96" height="96" aria-hidden="true"></canvas></span>';
    } else if (theme.orb3d === "sphere") {
      orbHtml =
        '<span class="research-card-orb research-card-orb--scan"><canvas class="research-card-orb-sphere" width="112" height="112" aria-hidden="true"></canvas></span>';
    }
  ...
  function buildModalBody(theme) {
    ...
    if (typeof colorizeLinks === "function") {
      colorizeLinks();
    }
  }
