...
(() => {
  ...
  async function waitForSize() {
    for (let i = 0; i < 40; i += 1) {
      ...
      // Polling frame by frame is the point here — the canvas has no size yet.
  ...
  function init() {
    ...
    initPromise = (async () => {
      ...
      if (!wasActive) {
        viewer.classList.add("preheating");
      }
      await waitForSize();
      ...
      try {
        ...
        if (e.message) {
          reportError(`NiiVue init failed: ${e.message}`);
        }
      } finally {
        if (!wasActive) {
  ...
  async function loadMod(mod) {
    ...
    const baseVol = {
      colormap: "gray",
      name: `${mod}.nii.gz`,
      url: blobUrls[mod] || mods[mod],
    };
    const seg = {
      colormap: "redyell",
      name: "seg.nii.gz",
      opacity: 0.6,
      url: segBlobUrl || segUrl,
    };
    try {
      ...
      try {
        ...
        reportError(`Volume load failed: ${e2.message ? e2.message : e2}`);
      }
    }
  }
  ...
  const warm = () => {
    init().catch(() => {
      // Warm-up is best effort; a real failure surfaces on activate().
    });
  };
  ...
  for (const el of document.querySelectorAll(
    'a[href="#research"], .research-link'
  )) {
    el.addEventListener("click", warm, { once: true });
  }

  const researchSection = document.getElementById("research");
  if (researchSection?.classList.contains("active")) {
    warm();
  }
})();
